...
def report_geraetelisten_old(request):
    ...
    for riege in riegen:
        for geraet in geraete:
            ...
            headers = ['Start-Nr.', 'Nachname', 'Vorname', 'Verein', 'Jahrgang', 'Meldung', 'A-Note', 'B-Note', 'Summe']
            table_data = [headers] + [list(row) + [''] * (len(headers) - len(row)) for row in teilnehmer_alle]

            if table_data:
                table = Table(table_data)

                # Stil für die Tabelle festlegen
                ...
                # Tabelle mit Stil rendern
                table.setStyle(style)

                table.wrapOn(p, breite, hoehe)
                table.drawOn(p, 1 * cm, 25 * cm)
...
def report_auswertung(request):
    ...
    for meisterschaft in meisterschaften:
        ...
        h = 1
        p.setFont('DejaVuSans-Bold', 18)
        p.drawCentredString(breite / 2, hoehe - (h * cm), "Ergebnisliste Bezirksturnfest 2024")
        h = h + 0.8
